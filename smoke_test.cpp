...
void check_constructors_polynom() {
    mp::polynomial p("5*x^3+2*x^1+6*x^0");
    ...
    assert(p.at(3) == 5);
    assert(p.at(4) == 0);
    assert(p.at(234567) == 0);

    assert(p.get_rank_polym() == 234567);

    mp::polynomial const p1("5*x^3+2*x^1+6*x^0");
    assert(p1.at(0) == 6);
    assert(p1.at(1) == 2);
    assert(p1.at(2) == 0);
    assert(p1.at(3) == 5);
    ...
    assert(p1.at(234567) == 0);
    assert(p1.get_rank_polym() == 3);
...
void run_bignum_zero_multiplication() {
    mp::bignum a(0);
    mp::bignum b(0);
...
int main() {
    ...
    run_bignum_zero_multiplication();
}
